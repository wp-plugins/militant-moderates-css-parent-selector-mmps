...
if(parsed.length)
$('<style type="text/css" mmps_ver="'+packageVersion+'">'+parsed+'</style>').appendTo('head');}else{}};navigator.browserInfo=(function(){var ua=navigator.userAgent,tem,M=ua.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i)||[];if(/trident/i.test(M[1])){tem=/\brv[ :]+(\d+)/g.exec(ua)||[];return'IE '+(tem[1]||'');}
...
var parseExternal=true;var parseInline=true;$('script[src*="'+packageName+'"]').each(function(){var src=$(this).attr('src').split('?');if(src[1]){var argv=src[1].split('&');for(var j=0;j<argv.length;++j){var pair=argv[j].split('=');if(pair[0]=='ver'){pair=pair[1].split('+');if(pair[0]!=packageVersion){}
if(pair[1]){if(pair[1].indexOf('X')<0)
parseExternal=false;if(pair[1].indexOf('I')<0)
parseInline=false;}else{parseExternal=parseInline=false;}}}}});$('link[rel="stylesheet"],style').each(function(i){if($(this).attr('mmps_ignore')){}else if($(this).is('link')){var href=$(this).attr('href');if(parseExternal||href.indexOf('display_mmps_ccss=css')>=0){$.ajax({type:'GET',url:href,dataType:'text'}).done(function(css){parse(css);}).fail(function(){});}}else if(parseInline||$(this).attr('mmps_ccss')){parse($(this).text());}});});