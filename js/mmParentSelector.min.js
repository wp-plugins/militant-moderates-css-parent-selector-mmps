...
if(subject){}else{}
break;default:subject=subject.closest(n);break;}
if(subject){}}while(subject&&n!=parent&&(n=$.trim(tmpNodes.pop())));if(subject){var id=CLASS+k++,ptNormState,toggleFn=function(e){if(e){if(pairedEventMap[e.type]){e.data.subject.on(pairedEventMap[e.type],function(oe){e.data.subject.toggleClass(e.data.id);e.data.subject.off(pairedEventMap[e.type]);});}
if(singleEventMap[e.type]){e.data.subject.off(singleEventMap[e.type]);}}
e.data.subject.toggleClass(e.data.id);};checkStateFn=function(e){if($(e.target).is(e.data.eState)){if(!($(subject).hasClass(id))){$(subject).toggleClass(id);}}else if($(subject).hasClass(id)){$(subject).toggleClass(id);}};idx&&(parsedSelectors.length)&&(parsedSelectors+=",");parsedSelectors+='.'+id;var qClass=false;if(pseudoParents.length){$(pseudoParents).each(function(pidx){var ppNorm=this.replace(/\-/g,'_');if(pseudoElements[ppNorm]){parsedSelectors+=pseudoElements[ppNorm];qClass=true;}else{$(subject).on(eventMap[this]||this,{id:id,subject:$(subject)},toggleFn);}});}
if(pseudoTargets.length){var orgChild=$(this);$(pseudoTargets).each(function(tidx){var ptNorm=this.replace(/\-/g,'_');var ptParam=null;if(/\(/.test(this)){var pcname=this.split(/(?:\(|\))/g);pcname[0]=pcname[0].replace(/\-/g,'_');if(stateParamMap[pcname[0]]){ptParam=stateParamMap[pcname[0]].replace(/\#/,pcname[1]);}}
if(pseudoElements[ptNorm]){}else if(eventMap[this]){qClass=false;$(orgChild).on(eventMap[this],{id:id,subject:$(subject)},toggleFn);}else if(stateMap[ptNorm]){ptNormState=stateMap[ptNorm];$(orgChild).on('click load visibilitychange',{eState:ptNormState},checkStateFn);if($(orgChild).is(ptNormState)){qClass=true;}else{qClass=false;}}else if(ptParam){if($(orgChild).is(ptParam)){qClass=true;}}else{qClass=false;$(orgChild).on(this.toString(),{id:id,subject:$(subject)},toggleFn);}});}else if(!pseudoParents.length){qClass=true;}
